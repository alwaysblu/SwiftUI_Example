    ...
    public func push<Element: View>(views:[(element: Element, identifier: String?)]) {
        ...
        withAnimation(easing) {
            ...
            for view in views {
              viewElements.append(
                ViewElement(
                  id: getIdentifier(view.identifier),
                  wrappedElement: AnyView(view.element)
                )
              )
            }
            viewStack.push(viewElements)
        }
    }
...
private struct ViewStack {
    ...
    /// add
    private func printDuplicateLog(id: String) -> Bool {
      guard indexForView(withId: id) == nil else {
        print("Duplicated view identifier: \"\(id)\". You are trying to push a view with an identifier that already exists on the navigation stack.")
        return false
      }
      return true
    }
    ...
    mutating func push(_ elements: [ViewElement]) {
      ...
      for element in elements {
        ...
        views.append(contentsOf: elements)
    }
